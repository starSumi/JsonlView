...
Internal acceptance also uses private operator data, but the source, size,
record count, identifiers, and measurements are intentionally not published.
They are not part of the public evidence set. Public reproducibility uses the
bounded synthetic corpus generated by `scripts/benchmark-newline-scanners.mjs`
and the engine benchmark fixtures. Those inputs cover high-record-count short
lines, long-line skew, LF/CRLF boundary splits, invalid UTF-8, no final newline,
partial-tail append, and dense-newline cases. Claims about 1 GiB, 10 GiB, or
50 GB remain unverified until those exact public inputs complete the same gate.
