...
The engine does not compute a full-file digest on the large-file open path.
That O(n) proof is started lazily when a refresh or snapshot guard requires an
exact append decision, and its result is shared by concurrent callers. This
keeps first-page latency bounded while retaining the conservative `unknown`
fallback when a writer moves during the proof.
...
Acceptance uses generated corpora rather than user or agent transcripts. The
public corpus matrix covers high-record-count short lines, long-line skew,
LF/CRLF boundary splits, invalid UTF-8, no final newline, and partial-tail
append. Claims about 1 GiB, 10 GiB, or 50 GB remain unverified until those exact
sizes complete the same gate.

Machine-specific timings and corpus fingerprints stay in external acceptance
artifacts. Re-run the benchmark commands in this document against a synthetic
fixture before making a performance claim. The standalone dense-newline case is
retained because the adaptive Number loop can beat the current N-API
implementation on that input shape; the choice is workload-dependent.
